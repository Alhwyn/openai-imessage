...
const resolveWaitOptions = (options: GenerateGmiImagesOptions): WaitOptions => ({
  ...
  pollIntervalMs: options.pollIntervalMs ?? IMAGE_POLL_INTERVAL_MS,
...
const parseResponse = async (response: Response): Promise<GmiImageResponse> => {
  ...
  if (!text.trim()) throw new Error(`GMI image API returned empty body (HTTP ${response.status})`);
...
const awaitImageResult = async (
  ...
  if (!requestId) throw new Error("GMI image API did not return a request_id to poll");

  const deadline = options.now() + options.timeoutMs;
  while (options.now() < deadline) {
    await options.sleep(options.pollIntervalMs);

    const payload = await gmiFetchJson(`${IMAGE_REQUESTS_URL}/${requestId}`, { method: "GET" }, fetchFn);
...
const generateOneImageUrl = async (
  ...
  if (!url) throw new Error("GMI image API returned no media URLs");
...
const downloadAlbum = async (
  ...
  try {
    for (const [index, url] of urls.entries()) {
      ...
      if (bytes.byteLength > IMAGE_MAX_FILE_BYTES) throw new Error(`Staged image too large (${(bytes.byteLength / 1_048_576).toFixed(2)} MiB)`);
