...
bun_bin=$(resolve_bun) || {
  echo "ci-check: bun not found (checked PATH, ${bun_install}/bin, ~/.bun/bin)" >&2
  echo "ci-check: install with: curl -fsSL https://bun.sh/install | bash" >&2
  exit 127
}
